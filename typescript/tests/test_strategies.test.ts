...
describe("EvolutionStrategy protocol", () => {
    ...
    test("PromptEvolver satisfies protocol", () => {
        const model = {
            complete: async () =>
                new Message({ role: MessageType.ASSISTANT, content: "{}" }),
        } as any;
...
describe("PromptEvolver", () => {
    ...
    test("propose returns code mods", async () => {
        ...
        const mods = await evolver.propose(
            trajectory,
            evalResult,
            {},
            { "system_prompt.py": "You are a helpful assistant." },
        );
    ...
    test("propose handles list response", async () => {
        ...
        const mods = await evolver.propose(
            ...
            evalResult,
            {},
            { "prompt_a.py": "old prompt", "prompt_b.py": "old prompt" },
        );
...
describe("ConfigEvolver", () => {
    ...
    test("low score proposes budget increase", async () => {
        ...
        expect(mods.length).toBeGreaterThanOrEqual(1);
        expect(
            mods.some((m) => m.description.toLowerCase().includes("budget")),
        ).toBe(true);
    });
    ...
    test("many steps proposes efficiency", async () => {
        ...
        const trajectory = new Trajectory();
        for (let i = 0; i < 60; i++)
            trajectory.add_step(new TrajectoryStep({ reward: 0.01 }));
        const evalResult = new EvalResult({
            passed: true,
            score: 0.8,
            reason: "ok but slow",
        ...
        expect(
            mods.some((m) =>
                m.description.toLowerCase().includes("efficiency"),
            ),
        ).toBe(true);
    });
    ...
    test("tool errors propose recovery", async () => {
        ...
        expect(mods.length).toBeGreaterThanOrEqual(1);
        expect(
            mods.some((m) => m.description.toLowerCase().includes("error")),
        ).toBe(true);
    });
});
...
describe("CompositeEvolutionStrategy", () => {
    ...
    test("aggregates", async () => {
        const strategyA = {
            name: "a",
            propose: async () => [
                new CodeMod({ target_file: "a.py", description: "mod from a" }),
            ],
        } as EvolutionStrategy;
        const strategyB = {
            name: "b",
            propose: async () => [
                new CodeMod({ target_file: "b.py", description: "mod from b" }),
            ],
        } as EvolutionStrategy;
        const composite = new CompositeEvolutionStrategy([
            strategyA,
        ...
        const mods = await composite.propose(
            new Trajectory(),
            new EvalResult({ passed: false, score: 0.3 }),
            {},
            {},
        );
        expect(mods.length).toBe(2);
...
describe("ConfigEvolver targeting (bug M4)", () => {
    test("targets an existing source file", async () => {
        ...
        expect(
            mods.every((m) => m.target_file === "loopengine/config.py"),
        ).toBe(true);
    });
});
...
describe("ToolEvolver", () => {
    test("creation", () => {
        const model = {} as any;
        const evolver = new ToolEvolver(model);
        expect((evolver as any)._model).toBe(model);
        expect(evolver.name).toBe("tool_evolver");
    });
    ...
    test("propose returns empty when no tool issues", async () => {
        const response = new Message({
            role: MessageType.ASSISTANT,
            content: "[]",
        });
        ...
        const evolver = new ToolEvolver(model);
        const trajectory = new Trajectory();
        const evalResult = new EvalResult({ passed: true, score: 0.95 });
        const mods = await evolver.propose(trajectory, evalResult, {}, {});
        expect(mods).toEqual([]);
    });

    test("propose returns CodeMods for problematic tools", async () => {
        const proposed_mods = JSON.stringify([
            {
                target_file: "tool_example.py",
                description: "Add error handling",
                diff: "--- tool_example.py\n+++ tool_example.py\n@@ -1,10 @@\n old code\n+new improved code\n",
                rationale: "High error rate in tool usage",
                expected_impact: "Reduce errors by 50%",
            },
        ]);
        const response = new Message({
            role: MessageType.ASSISTANT,
            content: proposed_mods,
        });
        const model = { complete: async () => response } as any;
        const evolver = new ToolEvolver(model);
        const trajectory = new Trajectory();
        const evalResult = new EvalResult({
            passed: false,
            score: 0.4,
            reason: "Tool errors detected",
        });
        const mods = await evolver.propose(
            trajectory,
            evalResult,
            {},
            { "tool_example.py": "example tool code" },
        );
        expect(mods.length).toBeGreaterThan(0);
        expect(mods[0].target_file).toBe("tool_example.py");
        expect(mods[0].description).toContain("error handling");
    });

    test("propose handles invalid model response", async () => {
        const response = new Message({
            role: MessageType.ASSISTANT,
        ...
        const evolver = new ToolEvolver(model);
        const trajectory = new Trajectory();
        const evalResult = new EvalResult({ passed: false, score: 0.3 });
        const mods = await evolver.propose(trajectory, evalResult, {}, {});
        expect(mods).toEqual([]);
    });
});
